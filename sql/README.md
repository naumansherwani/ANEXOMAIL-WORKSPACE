...
- `guard_account_integrity.sql` — (PHASE NUMBER NAHI — ye tumhari phase tarteeb se bahar ek guard hai, tumhari fair-use request se aayi) account integrity ladder (one person, one account): `account_integrity` state, append-only `account_integrity_log`, published `account_integrity_policy` (3 accounts/device · 3 signups/24h · 72h export), `account_integrity_evaluate` (deterministic flag only — engine never blocks), human-only `warn`/`block` (block needs a prior written warning) with device-hash-only bans, `export_ready`, `release`, and `purge_due` which only deletes after the 72 hour export window.
