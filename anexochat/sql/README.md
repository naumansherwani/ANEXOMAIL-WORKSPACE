...
- `phase31b_family_chat_workspace.sql` — founder + Humza + Raana ko ek real shared workspace mein rakhta hai aur founder↔Humza / founder↔Raana direct conversations idempotently ready karta hai. Existing messages/workspaces delete ya move nahi hotay; `chat_ensure_workspace()` in teen accounts ke liye shared family workspace ko pehle select karta hai.
