  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-6 py-8 md:px-8">
      <Section
        eyebrow={
          <>
            <MonitorSmartphone className="size-3.5" aria-hidden="true" /> Live sessions
          </>
        }
                    ...
                    <Button
                      ...
                      onClick={() =>
                        kill.mutate(
                          ...
                          {
                            onSuccess: () =>
                              notify.done("Session ended", "That device is signed out."),
                            onError: (e) =>
                              notify.failed(
                                e.isNotImplemented ? "Not wired yet" : "Could not end session",
                                {
      ...
      <section className="mt-10">
        <Section
          eyebrow={
            <>
              <Bomb className="size-3.5" aria-hidden="true" /> Blast-radius kill switch
            </>
          }
              ...
              blast.mutate(
                { reason: "Kill switch pulled from Security page" },
                {
                  ...
                  onError: (e) =>
                    notify.failed(
                      e.isNotImplemented
                        ? "Kill switch not wired yet"
                        : "Could not run kill switch",
                      {
                        description: e.message,
                      },
                    ),
