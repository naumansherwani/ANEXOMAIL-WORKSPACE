...
function DevicesPage() {
  ...
  const [cache, setCache] = useState<{ threads: number; thread: number; drafts: number } | null>(
    null,
  );
  ...
  return (
    <div className="mx-auto w-full max-w-3xl px-6 py-8 md:px-8">
      ...
      <h2 className="ax-heading mt-2 text-foreground">
        This device, and the ones you left mid-sentence
      </h2>
      <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
        Install it like an app, read your mail with no signal, and pick a draft back up exactly
      ...
      <section className="ax-plane mt-ax-4 rounded-2xl p-5">
        ...
        <p className="mt-3 text-[11px] text-muted-foreground">
          Currently resolved:{" "}
          <span className="text-foreground">{net.lowData ? "low data" : "full"}</span>.
        </p>
      </section>
                        ...
                        onClick={() =>
                          claim.mutate(
                            { id: draft.id },
                            {
                              ...
                              onError: (error) =>
                                notify.failed("Couldn't pick it up", {
