...
if [ -f "$SOURCE/mail-worker.ts" ]; then
  [ -f "$TARGET/src/mail-worker.ts" ] && cp -a "$TARGET/src/mail-worker.ts" "$TARGET/backups/$STAMP/mail-worker.ts"
  install -m 0644 "$SOURCE/mail-worker.ts" "$TARGET/src/mail-worker.ts"
fi
