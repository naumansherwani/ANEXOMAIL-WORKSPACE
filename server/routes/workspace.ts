...
workspaceRouter.post("/organisations", async (req, res) => {
  ...
  const { data, error } = await db
    .from("account_organisations")
    .insert({ name, slug, domain, created_by: uid })
    .select("id,name,slug,domain")
    .single();
  if (error) return res.status(500).json({ error: error.message });
  const { error: accountMemberError } = await db
    .from("account_org_members")
    .upsert({ org_id: data.id, user_id: uid, role: "owner" }, { onConflict: "org_id,user_id" });
  ...
  const { error: legacyOrgError } = await db
    .from("orgs")
    .upsert({ id: data.id, name }, { onConflict: "id" });
  if (legacyOrgError) {
    await db.from("account_org_members").delete().eq("org_id", data.id).eq("user_id", uid);
    await db.from("account_organisations").delete().eq("id", data.id);
    return res.status(500).json({ error: "operational_workspace_failed" });
  }
  const { error: legacyMemberError } = await db
    .from("org_members")
    .upsert(
  ...
  if (legacyMemberError) {
    await db.from("orgs").delete().eq("id", data.id);
    await db.from("account_org_members").delete().eq("org_id", data.id).eq("user_id", uid);
    await db.from("account_organisations").delete().eq("id", data.id);
    return res.status(500).json({ error: "operational_membership_failed" });
  }
...
workspaceRouter.post("/invitations", async (req, res) => {
  ...
  for (const raw of emails) {
    ...
    const { error } = await db!.auth.admin.inviteUserByEmail(email, {
      ...
      data: { organisation_id: membership.org_id, role: "member" },
    });
