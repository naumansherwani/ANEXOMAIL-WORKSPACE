...
async function saveSession(userId: string, token: string, req: any) {
  ...
  await getAdmin().from("account_sessions").upsert({
    user_id: userId,
...
authRouter.post("/signup", async (req, res) => {
  ...
  await getAdmin().from("user_roles").upsert({ user_id: data.user.id, role: "user" }, { onConflict: "user_id,role" });
  await getAdmin().rpc("trial_start", { _user_id: data.user.id, _social_email: email, _provider: "email" });
...
authRouter.post("/logout", async (req, res) => {
  const identity = await userFrom(req, res); if (!identity) return;
  await getAdmin().from("account_sessions").update({ revoked_at: new Date().toISOString() }).eq("token_hash", tokenHash(identity.token));
...
authRouter.post("/change-password", async (req, res) => {
  ...
  const { error } = await getAdmin().auth.admin.updateUserById(identity.user.id, { password: newPassword });
  if (error) return authError(res, error);
  await getAdmin().from("account_sessions").update({ revoked_at: new Date().toISOString() }).eq("user_id", identity.user.id).neq("token_hash", tokenHash(identity.token));
  res.json({ ok: true });
});
...
authRouter.get("/sessions", async (req, res) => {
  ...
  const { data, error } = await getAdmin().from("account_sessions").select("id,token_hash,device,browser,ip,location,last_seen_at").eq("user_id", identity.user.id).is("revoked_at", null).order("last_seen_at", { ascending: false });
...
authRouter.delete("/sessions/:id", async (req, res) => {
  const identity = await userFrom(req, res); if (!identity) return;
  const { error } = await getAdmin().from("account_sessions").update({ revoked_at: new Date().toISOString() }).eq("id", req.params.id).eq("user_id", identity.user.id);
...
authRouter.post("/onboarding/complete", async (req, res) => {
  const identity = await userFrom(req, res); if (!identity) return;
  const { error } = await getAdmin().from("account_profiles").update({ onboarded: true, updated_at: new Date().toISOString() }).eq("user_id", identity.user.id);
...
authRouter.get("/identity/check", async (req, res) => {
  ...
  const [{ data: reserved }, { data: taken }] = await Promise.all([
    ...
    getAdmin().from("trial_accounts").select("user_id").eq("anexomail_handle", username).neq("status", "released").maybeSingle(),
  ]);
...
authRouter.post("/identity/claim", async (req, res) => {
  ...
  const { data, error } = await getAdmin().rpc("trial_claim_address", { _user_id: identity.user.id, _handle: username });
